...
class Chart:
    def __init__(self, chart_title='', chart_source='', y_label='', x_label='Date', x_years=5):
    ...
    def basic_chart(self, df: pd.DataFrame):
        ...
        start_year = (df.index[0].year // self.x_years) * self.x_years
        end_year = (df.index[-1].year // self.x_years) * self.x_years + self.x_years
        ...
        ax.xaxis.set_major_locator(mdates.YearLocator(self.x_years))  # Set major ticks every x years
